...
class Agent(ABC):
    ...
    def __init__(
        ...
        self._history_token_count = 0 # 缓存历史token数
    ...
    def _should_compress(self) -> bool:
        ...
        Returns:
            是否需要压缩
        """
        threshold = int(self.config.compression_threshold * self.config.context_window)
    ...
    def _execute_tool_call(self, tool_name: str, arguments: Dict[str, Any]) -> str:
        ...
        if tool:
            try:
                ...
                response = self.tool_registry.execute_tool(tool_name, typed_arguments)
    ...
    def _auto_save(self):
        ...
        try:
            self.session_store.save(
                ...
                history=self.history_manager.get_history(),
                tool_schema_hash=self.compute_tool_schema_hash()
            )
        except Exception as e:
            # 自动保存失败不影响主流程
